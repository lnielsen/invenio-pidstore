...
class PersistentIdentifier(db.Model, Timestamp):
    ...
    def assign(self, object_type, object_uuid, overwrite=False):
        ...
        if existing_obj_id and existing_obj_id != object_uuid:
            if not overwrite:
                raise PIDObjectAlreadyAssigned(object_type,
                                               object_uuid)
            logger.info("Unassigned object {0}:{1}".format(
                object_type, existing_obj_id), extra=dict(pid=self))
        elif existing_obj_id and existing_obj_id == object_uuid:
